...
class TestImportExportFlow:
    ...
    def test_export_colony(self, auth_client, tmp_path: Path):
        ...
        export_response = auth_client.post(
            "/api/v1/colony-tools/export",
            json={"colony_id": colony_id, "output_path": str(export_path)},
        )
    ...
    def test_import_colony(self, auth_client, tmp_path: Path):
        ...
        import_response = auth_client.post(
            "/api/v1/colony-tools/import", json={"input_path": str(import_path)}
        )
        # Note: This endpoint might not exist yet - adjust based on actual implementation
    ...
    def test_export_import_roundtrip(self, auth_client, tmp_path: Path):
        ...
        export_response = auth_client.post(
            "/api/v1/colony-tools/export",
            json={"colony_id": original["id"], "output_path": str(export_path)},
        )

        # Import (would create a new colony)
        import_response = auth_client.post(
            "/api/v1/colony-tools/import", json={"input_path": str(export_path)}
        )
