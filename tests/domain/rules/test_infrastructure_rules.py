...
class TestInfrastructureRules:
    ...
    def test_transport_working(self):
        ...
        assert len(mods) == 2
        assert any(
            m.modifier_stat == ModifierStat.PRODUCTIVITY and m.modifier_value == 1 for m in mods
        )
        assert any(
            m.modifier_stat == ModifierStat.COMPLACENCY and m.modifier_value == 1 for m in mods
        )
    ...
    def test_transport_not_working(self):
        ...
        assert len(mods) == 2
        assert any(
            m.modifier_stat == ModifierStat.PRODUCTIVITY and m.modifier_value == -2 for m in mods
        )
    ...
    def test_power_network_not_working(self):
        ...
        assert len(mods) == 2
        assert any(
            m.modifier_stat == ModifierStat.PRODUCTIVITY and m.modifier_value == -3 for m in mods
        )
        assert any(
            m.modifier_stat == ModifierStat.COMPLACENCY and m.modifier_value == -1 for m in mods
        )
    ...
    def test_water_management_not_working(self):
        ...
        assert any(m.modifier_stat == ModifierStat.ORDER and m.modifier_value == -2 for m in mods)
        assert any(
            m.modifier_stat == ModifierStat.COMPLACENCY and m.modifier_value == -2 for m in mods
        )

    def test_food_production_working(self):
        infra = Infrastructure(
            colony_id=1,
            infrastructure_type=InfrastructureType.FOOD_PRODUCTION,
            state=InfrastructureState.WORKING,
        )
        mods = get_infrastructure_modifiers(infra)
        assert len(mods) == 2
