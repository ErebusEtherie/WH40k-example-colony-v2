...
7. **Add event system queries** — "is a roll due?", "upcoming rolls" endpoints
8. **Implement modifier expiry logic** — automatic time-based removal

### Long-term (Phase 5+)
9. **Wire Representative Type bonuses** — mechanical link to PF or other stats
10. **Wire Personality effects** — calculate bonuses based on assigned personalities
11. **Add user authentication** — API security if deployed publicly
12. **Add data export formats** — CSV, PDF reports for GMs
