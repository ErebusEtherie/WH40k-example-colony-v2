...
class UserService:
    ...
    def update_user(
        ...
        if role is not None:
            old_role = target_user.role.value if isinstance(target_user.role, UserRole) else target_user.role
            new_role = role
    ...
    def reset_password(
        ...
        if changed_by is not None:
            ...
            logger.warning("Password reset for user %d performed without a valid modifier (system operation)", user_id)
