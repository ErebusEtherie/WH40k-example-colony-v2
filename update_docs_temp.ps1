...
# Update DECISIONS_AND_QUESTIONS.md
decisions_content = '''# Architectural Decisions & Open Questions
...
- pplication/ — use cases/services, orchestrates domain + ports
- dapters/ — persistence, API, CLI, IO implementations
...
**Decision:** Define repository interfaces in domain/ports/, implement in dapters/persistence/ against SQLite.
...
**Decision:** Per the rules reference document, the Colony Manager does NOT implement dice rolls or automated event systems.
...
- Acquisition Tests, skill checks, etc. are not performed by the app
- GM Control via Custom Modifiers allows situational bonuses/penalties

**Status:** Implemented - domain is pure calculation engine, no randomness

---

### 8. Application Service Tests Complete (2026-08-24)

**Decision:** All application service tests are implemented and passing.

**Files:**

- tests/application/services/test_infrastructure_service.py - Complete
- tests/application/services/test_support_upgrade_service.py - Complete

**Status:** Implemented - 640 tests passing
...
- **A:** One user to one colony (simpler, current design)
- **B:** One user to many colonies via separate ColonyMembership table
...
**Status:** Pending - cleanup CLI command exists, no automatic policy
...
Path('d:/Projekty/WH40k_Colony_Manager/DECISIONS_AND_QUESTIONS.md').write_text(decisions_content, encoding='utf-8')
print('Updated DECISIONS_AND_QUESTIONS.md')
